...
public class FeigeFiatShamirIdentification {
    ...
    public void execute() {
        ...
        RAndZ rAndZ;
        System.out.println("А calculating z = " + (rAndZ = getRandomRAndZ(n)).z + " and sending z to B.");
        int b;
        System.out.println("B sending А random bit b = " + (b = new Random().nextInt(2)));
        if (b == 0) {
            System.out.println("А send to B r = " + rAndZ.r);
            System.out.println("B checking z = r^2 mod n and result = " + (rAndZ.z == BigInteger.valueOf(rAndZ.r).pow(2).mod(BigInteger.valueOf(n)).intValue()));
        } else {
            int y;
            System.out.println("А sending B y = " + (y = BigInteger.valueOf(rAndZ.r).multiply(BigInteger.valueOf(s)).mod(BigInteger.valueOf(n)).intValue()));
            System.out.println("B checking z = (y^2 * v) mod n and result = " + (rAndZ.z == BigInteger.valueOf(y).pow(2).multiply(BigInteger.valueOf(quadraticSquare.v)).mod(BigInteger.valueOf(n)).intValue()));
        }
        System.out.println("\n");
    }
